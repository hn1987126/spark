...
object AdvUserLocation {
  ...
  def main(args: Array[String]) {
    ...
    val rdd0 = sc.textFile(this.getClass().getResource("/logfile/userlocal/bs_log").toString).map( line => {
    ...
    val rdd2 = sc.textFile(this.getClass().getResource("/logfile/userlocal/loc_info.txt").toString).map(line => {
      val f = line.split(",")
      //(基站ID， （经度，纬度）)
      (f(0), (f(1), f(2)))
    })
